...
class WorkWithMePage(AbstractEmailForm):
    ...
    # Optional downloadable vCard file (typically absolute URL on Cloudinary)
    ...
    def _absolute_url(self, path_or_url: str) -> str:
        """
        Ensure we return an absolute URL. Cloudinary gives absolute URLs already.
        Local storage may return a path like `/media/...`.
        """
        if not path_or_url:
            return ""
    ...
    def get_qr_payload(self) -> str:
        """
        Choose the shortest, most reliable payload for scanning:
        1) vCard file URL (if uploaded)
        2) Manually-entered qr_data (verbatim)
        3) mailto: or tel:
        4) Full page URL
        """
        if self.vcard_file:
        ...
        # Wagtail Page gives a full absolute URL if Site is configured
        try:
            return self.get_full_url()
        except Exception:
            # As a last resort, return relative URL
            return self.url or "/"
    ...
    def save(self, *args, **kwargs):
        ...
        # Seed a basic form on first create (idempotent)
